    ...
    private fun playVideo(isEndOfList: Boolean, recyclerView: RecyclerView){
        ...
        if (!isEndOfList) {
            ...
            targetPosition = if (startPosition != endPosition) {
                val startPositionVideoHeight: Int = getVisibleVideoSurfaceHeight(startPosition, recyclerView)
                val endPositionVideoHeight: Int = getVisibleVideoSurfaceHeight(endPosition, recyclerView)
                if (startPositionVideoHeight > endPositionVideoHeight) startPosition else endPosition
            } else {
                startPosition
            }
        } else {
            targetPosition = (recyclerView.adapter  as VideoPlayerRecyclerAdapter).getData().size - 1
        }
        ...
        val mediaUrl = (recyclerView.adapter as VideoPlayerRecyclerAdapter).getData()[targetPosition].media_url
